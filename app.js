...
function switchTab(tab) {
  ...
  $("#fabBtn").classList.toggle("hidden", tab !== "chats");
...
function renderMessageNode(m) {
  ...
  if (m.type === "text") {
    ...
    bubble.innerHTML = `<video class="msg-video" controls src="${m.file_url}"></video><span class="time">${formatTime(m.created_at)}</span>`;
...
/* Realtime: saat profil siapa pun berubah (foto, about, online), sinkronkan di layar */
function subscribeToProfileChanges() {
  supabase
    ...
    .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles" }, (payload) => {
      ...
      const chatRow = state.chats.find((c) => c.peer?.id === updated.id);
      if (chatRow) {
        chatRow.peer = { ...chatRow.peer, ...updated };
        renderChatList($("#searchInput").value);
      }

      if (state.activePeer?.id === updated.id) {
        state.activePeer = { ...state.activePeer, ...updated };
        fillAvatarNode($("#peerAvatar"), state.activePeer);
        $("#peerName").textContent = state.activePeer.username || "Pengguna";
        $("#peerStatus").textContent = state.activePeer.is_online ? "online" : "offline";
      }
    })
    .subscribe();
}
...
async function uploadAndSend(file, kindHint) {
  ...
  const kind = kindHint === "media" ? (file.type.startsWith("video/") ? "video" : "image") : kindHint;
  toast(kind === "video" ? "Mengunggah video..." : kind === "image" ? "Mengunggah gambar..." : "Mengunggah file...");
...
const stickerMaker = { img: null, zoom: 1, offsetX: 0, offsetY: 0, dragging: false, lastX: 0, lastY: 0 };
...
/* =========================================================
   PEMBARUAN (status foto/video/teks, 24 jam, bisa dilihat)
========================================================= */
async function loadStatuses() {
  ...
  feed.innerHTML = "";
  const others = state.statuses.filter((s) => s.owner_id !== state.me.id);
  if (!others.length) {
  ...
  others.forEach((s) => {
    ...
    const mediaLabel = s.type === "image" ? "📷 Foto" : s.type === "video" ? "🎥 Video" : escapeHtml(s.content || "");
    div.innerHTML = `<div class="row-text"><strong>${escapeHtml(s.profiles?.username || "Pengguna")}</strong><span>${mediaLabel}</span></div>`;
    div.prepend(avatar);
    div.addEventListener("click", () => openStatusViewer(s));
    feed.appendChild(div);
  });
}

function openStatusViewer(status) {
  fillAvatarNode($("#viewerAvatar"), status.profiles || state.me);
  $("#viewerName").textContent = status.profiles?.username || state.me.username;
  $("#viewerTime").textContent = formatTime(status.created_at) + " · " + formatDay(status.created_at);
  ...
  if (status.type === "image") {
    ...
    if (status.content) {
      ...
      p.textContent = status.content;
      body.appendChild(p);
    }
  } else if (status.type === "video") {
    const video = document.createElement("video");
    video.src = status.image_url;
    video.controls = true;
    video.autoplay = true;
...
function openMyStatusOrComposer() {
  const mine = state.statuses.find((s) => s.owner_id === state.me.id);
  if (mine) openStatusViewer(mine);
  else openStatusComposer();
}
...
function resetStatusPreview() {
  state.pendingStatusFile = null;
  $("#statusMediaInput").value = "";
  $("#statusPreviewWrap").classList.add("hidden");
  $("#statusPreviewImg").classList.add("hidden");
  $("#statusPreviewVideo").classList.add("hidden");
  $("#statusPreviewImg").src = "";
  $("#statusPreviewVideo").src = "";
}

function handleStatusMediaChange(file) {
  if (!file) return;
  state.pendingStatusFile = file;
  const isVideo = file.type.startsWith("video/");
  const url = URL.createObjectURL(file);
  $("#statusPreviewWrap").classList.remove("hidden");
  if (isVideo) {
    $("#statusPreviewVideo").src = url;
    $("#statusPreviewVideo").classList.remove("hidden");
    $("#statusPreviewImg").classList.add("hidden");
  } else {
    $("#statusPreviewImg").src = url;
    $("#statusPreviewImg").classList.remove("hidden");
    $("#statusPreviewVideo").classList.add("hidden");
  }
}
...
async function postStatus() {
  ...
  const file = state.pendingStatusFile;
  if (!text && !file) { toast("Tulis sesuatu atau tambahkan media dulu"); return; }

  $("#postStatusBtn").disabled = true;
  $("#postStatusBtn").textContent = "Membagikan...";
  ...
  try {
    ...
    if (file) {
      ...
      const path = `status/${state.me.id}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
      const { error: upErr } = await supabase.storage.from(BUCKET_MEDIA).upload(path, file);
      if (upErr) throw upErr;
      const { data: pub } = supabase.storage.from(BUCKET_MEDIA).getPublicUrl(path);
    ...
    console.error(err);
    toast("Gagal membagikan pembaruan");
  } finally {
    $("#postStatusBtn").disabled = false;
    $("#postStatusBtn").textContent = "Bagikan";
  }
}
...
function wireEvents() {
  ...
  const openNewChatModal = () => { $("#newChatSearch").value = ""; $("#newChatResults").innerHTML = ""; openModal("newChatModal"); $("#newChatSearch").focus(); };
  ...
  document.addEventListener("click", (e) => {
    if (!$("#dotsMenu").contains(e.target) && e.target.closest("#menuDotsBtn") === null) $("#dotsMenu").classList.add("hidden");
    if (!$("#attachMenu").contains(e.target) && e.target.closest("#attachBtn") === null) $("#attachMenu").classList.add("hidden");
  });
  ...
  $("#myStatusRow").addEventListener("click", openMyStatusOrComposer);
  $("#postStatusBtn").addEventListener("click", postStatus);
  $("#statusMediaInput").addEventListener("change", (e) => handleStatusMediaChange(e.target.files[0]));
  $("#statusPreviewRemove").addEventListener("click", resetStatusPreview);
  ...
  $$("#attachMenu button[data-type]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (type === "media") $("#mediaInput").click();
      if (type === "file") $("#fileInput").click();
      if (type === "sticker") { resetStickerMaker(); openModal("stickerMakerModal"); }
    });
  });
  $("#mediaInput").addEventListener("change", (e) => { uploadAndSend(e.target.files[0], "media"); e.target.value = ""; });
